# Does the same thing as best.R except it allows for choice of rank

rankhospital <- function(state, outcome, num = "best") {
        ## Read outcome data
        outcome_data <- read('~/Documents/Coursera/RProgramming/Assignment3/rprog_data_ProgAssignment3-data/outcome-of-care-measures.csv')
        
        ## Read hospital data
        hospital_data <- read('~/Documents/Coursera/RProgramming/Assignment3/rprog_data_ProgAssignment3-data/hospital-data.csv')
        
        ## Check that state is valid
        # first build a set of states combining the list of outcomes and hospitals
        all_states <- sort(unique((c(as.character(unique(hospcare$State)), as.character(unique(outcome$State))))))
        ...
        mortality_data <- subset.data.frame(x = outcome,select = c('State', 'Hospital.Name', names(outcome)[grepl(pattern = "^Hospital.30.Day.Death..Mortality..Rates.from.", x = names(outcome))]))
        ...
        matched_mort_col <- paste("Hospital.30.Day.Death..Mortality..Rates.from.",matched_outcome,sep = "")
        # we remove the rows that don't have any data
        mort_for_state <- mort_for_state[!is.na(as.numeric(as.character(mort_for_state$matched_mort_col))),]
        
        if( num > length(mort_for_state) ){
                num = -1
        }
        
        # sort by mort rate ascending (lower is better) and hospital name alphabetically
        # and select the desired rank
        ranked_hospital <- mort_for_state[order(mort_for_state$matched_mort_col, mort_for_state$Hospital.Name),][num,'Hospital.Name']
        ranked_hospital
}
